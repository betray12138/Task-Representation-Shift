    ...
    for i, task in enumerate(tasks):
        ...
        if obs.dim() < 3:
            if args.env_name == 'WalkerRandParams-v0':
                traj_size = obs.shape[0] // args.trajectory_len * args.trajectory_len
                obs, actions,rewards,next_obs,terminals = obs[:traj_size], actions[:traj_size],rewards[:traj_size],next_obs[:traj_size],terminals[:traj_size]
