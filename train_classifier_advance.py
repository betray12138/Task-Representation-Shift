...
class FOCAL:
	...
	def __init__(self, args, train_dataset, train_goals, eval_dataset, eval_goals):
		...
		# self.context_classifier.load_state_dict(torch.load("logs/AntDir-v0/half_update/models/context_classifier200.pt", map_location=ptu.device))
	...
	def update(self, tasks, _iter):
		...
		for update in range(self.args.rl_updates_per_iter): 
			...
			for i in range(self.args.update_advance_frequency): 
				context = self.sample_context_batch_finetune(tasks, self.storage) 
				classifier_encoding, _, classifier_loss = self.update_classifier(tasks, context)
			...
			# new_action, _, _, next_log_prob = self.agent.act(obs_c, return_log_prob=True)
			# div_estimate = self._divergence.dual_estimate(
			# 	obs_c, new_action, actions, False)
			# c_loss = self._divergence.dual_critic_loss(obs_c, new_action, actions)  
			# self.c_optim.zero_grad()
			# c_loss.backward(retain_graph=True)
			# self.c_optim.step()
			# for _ in range(self.args.c_iter - 1):  
			# 	self._optimize_c(indices=tasks, context=classifier_encoding)
