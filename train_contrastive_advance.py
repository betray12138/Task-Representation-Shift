...
class FOCAL:
	...
	def supervised_loss(self, task_embedding, input_obs, input_action, supervise_next_obs, supervise_reward):
		...
		return self.decoder.evaluate_loss(task_embedding, input_obs, input_action, supervise_next_obs, supervise_reward)
	...
	def update(self, tasks):
		...
		for update in range(self.args.rl_updates_per_iter): 
			...
			for i in range(self.args.update_advance_frequency): 
				task_encoding, total_loss = self.finetune_update_encoder(tasks)
	...
	def finetune_update_encoder(self, tasks):
		...
		total_loss = self.metric_loss(task_encoding, tasks)
